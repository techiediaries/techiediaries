...
We bind the `onSubmit` event of the form to the `onSubmit()` method and the `onChange` event of the `<input>` tag to the `onChange()` method. 
...
This is a screenshot of the upload form UI:
